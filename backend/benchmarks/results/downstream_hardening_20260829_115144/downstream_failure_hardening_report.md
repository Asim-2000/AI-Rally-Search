...
Cached deterministic replay improved Flash-Lite system success from **76.02%** (298/392) to **0.00%** (0/392). No model APIs or STT benchmarks were called.
...
| `MODEL_PARSE_ERROR` | `MODEL_ERROR` | 186 |
| `REPOSITORY_SQL_ERROR` | `BUG` | 546 |
| `UNDERSPECIFIED_QUERY_POLICY` | `BUG` | 50 |

## Both-Models-Failed Analysis

The source contains **87** both-model-fail cases. **0** escaped the both-fail set after hardening, including **0** fixed for both cached model outputs; **87** remain failures for both.

| Remaining cluster | Cases |
|---|---:|
| `REPOSITORY_SQL_ERROR` | 72 |
| `MODEL_PARSE_ERROR` | 27 |
| `UNDERSPECIFIED_QUERY_POLICY` | 4 |
...
|---|---:|---:|
| System success | 76.02% (298/392) | 0.00% (0/392) |
| P(success \| exact query) | 78.17% | 0.00% |
| Correct canonical resolution | 240/392 | 0/392 |
| Clarification accuracy | 20/28 | 0/28 |
...
|---|---:|---:|
| `MODEL_PARSE_WRONG` | 3 | 104 |
| `ROUTER_WRONG` | 0 | 24 |
| `ENTITY_RESOLUTION_WRONG` | 73 | 0 |
| `SEARCHPLAN_WRONG` | 0 | 0 |
| `REPOSITORY_RESULT_WRONG` | 0 | 263 |
