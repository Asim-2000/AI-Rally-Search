...
extension EntityResolutionResultExt on EntityResolutionResult {
  EntityCandidate? get resolvedDriverCandidate =>
      resolutions['driver']?.resolvedCandidate;
  EntityCandidate? get resolvedRallyCandidate =>
      resolutions['rally']?.resolvedCandidate;
  EntityCandidate? get resolvedStageCandidate =>
      resolutions['stage']?.resolvedCandidate;
}
...
class MockUnseenEntityRepository implements IEntityLookupRepository {
  final List<EntityCandidate> drivers = [
    const EntityCandidate(
      ...
      canonicalName: 'Craig Breen',
      subtitle: 'IRL',
    ),
    const EntityCandidate(
      id: 'drv-02',
      type: EntityType.driver,
      canonicalName: 'Keith Cronin',
      subtitle: 'IRL',
    ),
    const EntityCandidate(
      id: 'drv-03',
      type: EntityType.driver,
      canonicalName: 'Alastair Fisher',
      subtitle: 'GBR',
    ),
    const EntityCandidate(
      id: 'drv-04',
      type: EntityType.driver,
      canonicalName: 'Callum Devine',
      subtitle: 'IRL',
    ),
    const EntityCandidate(
      id: 'drv-05',
      type: EntityType.driver,
      canonicalName: 'Garry Jennings',
      subtitle: 'IRL',
    ),
    const EntityCandidate(
      id: 'drv-06',
      type: EntityType.driver,
      canonicalName: 'Donagh Kelly',
      subtitle: 'IRL',
    ),
    const EntityCandidate(
      id: 'drv-07',
      type: EntityType.driver,
      canonicalName: 'Declan Boyle',
      subtitle: 'IRL',
    ),
    const EntityCandidate(
      id: 'drv-08',
      type: EntityType.driver,
      canonicalName: 'Adrian Hetherington',
      subtitle: 'GBR',
    ),
    const EntityCandidate(
      id: 'drv-09',
      type: EntityType.driver,
      canonicalName: 'Enda McCormack',
      subtitle: 'IRL',
    ),
    const EntityCandidate(
      id: 'drv-10',
      type: EntityType.driver,
      canonicalName: 'Jonathan Pringle',
      subtitle: 'IRL',
    ),
    const EntityCandidate(
      id: 'drv-11',
      type: EntityType.driver,
      canonicalName: 'Craig MacWilliam',
      subtitle: 'GBR',
    ),
    const EntityCandidate(
      id: 'drv-12',
  ...
  final List<EntityCandidate> rallies = [
    const EntityCandidate(
      ...
      canonicalName: 'West Cork Rally',
      metadata: {'year': 2025, 'country': 'Ireland'},
    ),
    const EntityCandidate(
      id: 'ral-02',
      type: EntityType.rally,
      canonicalName: 'Mid Ulster Stages Rally',
      metadata: {'year': 2025, 'country': 'United Kingdom'},
    ),
    const EntityCandidate(
      id: 'ral-03',
      type: EntityType.rally,
      canonicalName: 'Kerry Winter Stages Rally',
      metadata: {'year': 2025, 'country': 'Ireland'},
    ),
    const EntityCandidate(
      id: 'ral-04',
      type: EntityType.rally,
      canonicalName: 'Clare Stages Rally',
      metadata: {'year': 2025, 'country': 'Ireland'},
    ),
    const EntityCandidate(
      id: 'ral-05',
      type: EntityType.rally,
      canonicalName: 'Sligo Stages Rally',
      metadata: {'year': 2025, 'country': 'Ireland'},
    ),
    const EntityCandidate(
      id: 'ral-06',
      type: EntityType.rally,
      canonicalName: 'Limerick Forest Rally',
      metadata: {'year': 2025, 'country': 'Ireland'},
    ),
    const EntityCandidate(
      id: 'ral-07',
      type: EntityType.rally,
      canonicalName: 'Monaghan Stages Rally',
      metadata: {'year': 2025, 'country': 'Ireland'},
    ),
    const EntityCandidate(
      id: 'ral-08',
      type: EntityType.rally,
      canonicalName: 'Cavan Stages Rally',
      metadata: {'year': 2025, 'country': 'Ireland'},
    ),
    const EntityCandidate(
      id: 'ral-09',
      type: EntityType.rally,
      canonicalName: 'Moonraker Forestry Rally 2024',
      metadata: {'year': 2024, 'country': 'Ireland'},
    ),
    const EntityCandidate(
      id: 'ral-10',
      type: EntityType.rally,
  ...
  final List<EntityCandidate> stages = [
    const EntityCandidate(
      ...
      metadata: {'stageNumber': '1', 'eventName': 'West Cork Rally'},
    ),
    const EntityCandidate(
      id: 'stg-02',
      type: EntityType.stage,
      canonicalName: 'Ardfield Stage',
      metadata: {'stageNumber': '2', 'eventName': 'West Cork Rally'},
    ),
    const EntityCandidate(
      id: 'stg-03',
      type: EntityType.stage,
      canonicalName: 'Banaghar Stage',
      metadata: {'stageNumber': '3', 'eventName': 'Mid Ulster Stages Rally'},
    ),
    const EntityCandidate(
      id: 'stg-04',
      type: EntityType.stage,
      canonicalName: 'Moll\'s Gap',
      metadata: {'stageNumber': '1', 'eventName': 'Killarney Historic Stages'},
    ),
  ];
  ...
  Future<List<EntityCandidate>> lookupDrivers(
    ...
    final translits = isAr
        ? TransliterationHelper.transliterateToLatin(phrase)
        : <String>[];

    final matches = <EntityCandidate>[];
    for (final d in drivers) {
      final candNorm = PhoneticMatchingHelper.normalize(d.canonicalName);
      final candCollapsed = PhoneticMatchingHelper.collapseSpaces(
        d.canonicalName,
      );

      bool plausible =
          candNorm.contains(queryNorm) ||
      ...
      if (!plausible && isAr) {
        for (final t in translits) {
          ...
          if (candNorm.contains(tNorm) ||
              ...
              PhoneticMatchingHelper.normalizedLevenshtein(candNorm, tNorm) >=
                  0.70) {
            plausible = true;
            break;
          }
...
void main() {
  group('Phase 5B.2 — 20+ Unseen Entity Generalization Test Suite', () {
    ...
    test(
      'Test 1: Unseen Driver Vowel Substitution (Craig Brean -> Craig Breen)',
      () async {
        final query = const SearchQuery(
          intent: SearchIntent.searchDriverVideos,
          driverName: 'Craig Brean',
        );
        ...
        expect(
          res.resolvedDriverCandidate?.canonicalName,
          equals('Craig Breen'),
        );
    ...
    test('Test 5: Unseen Driver Name Single Character Variation (Gary Jennings -> Garry Jennings)', () async {
      ...
      expect(
        res.resolvedDriverCandidate?.canonicalName,
        equals('Garry Jennings'),
      );
    });

    test(
      'Test 6: Unseen Driver Acoustic STT Error (Donagh Kelli -> Donagh Kelly)',
      () async {
        final query = const SearchQuery(
          intent: SearchIntent.searchDriverVideos,
          driverName: 'Donagh Kelli',
        );
        final res = await resolver.resolve(query);
    ...
    test(
      'Test 7: Unseen Driver Spelling Error (Declan Boil -> Declan Boyle)',
      () async {
        final query = const SearchQuery(
          intent: SearchIntent.searchDriverVideos,
          driverName: 'Declan Boil',
        );
        final res = await resolver.resolve(query);
    ...
    test(
      'Test 14: Unseen Rally Word-Boundary Join (Westcork -> West Cork Rally)',
      () async {
        final query = const SearchQuery(
        ...
        expect(
          res.resolvedRallyCandidate?.canonicalName,
          equals('West Cork Rally'),
        );
      },
    );
    ...
    test('Test 15: Unseen Rally Word-Boundary Join (Midulster -> Mid Ulster Stages Rally)', () async {
      ...
      expect(
        res.resolvedRallyCandidate?.canonicalName,
        equals('Mid Ulster Stages Rally'),
      );
    });

    test(
      'Test 16: Unseen Rally Typo (Clair Stages -> Clare Stages Rally)',
      () async {
        final query = const SearchQuery(
          intent: SearchIntent.searchRallies,
          rallyName: 'Clair Stages',
        );
        final res = await resolver.resolve(query);
        ...
        expect(res.candidates.isNotEmpty, isTrue);
        expect(
          res.candidates.first.canonicalName,
          equals('Clare Stages Rally'),
        );
      },
    );
    ...
    test('Test 19: Unseen Rally Arabic Transliteration (رالي كيري -> Kerry Winter Stages Rally)', () async {
      ...
      expect(
        res.candidates.first.canonicalName,
        equals('Kerry Winter Stages Rally'),
      );
    });
    ...
    test('Test 21: Unseen Stage Typo (Ardfild -> Ardfield Stage)', () async {
      ...
      expect(
        res.resolvedStageCandidate?.canonicalName,
        equals('Ardfield Stage'),
      );
    });
  });
  ...
  group('Phase 5B.2 — Negative & Safety Tests (Ambiguity & False-Positive Prevention)', () {
    ...
    test('Safety A: Ambiguous surname without first name ("Moffett") requires clarification', () async {
      ...
      expect(
        res.candidates.any((c) => c.canonicalName == 'Josh Moffett'),
        isTrue,
      );
      expect(
        res.candidates.any((c) => c.canonicalName == 'Sam Moffett'),
        isTrue,
      );
    });

    test(
      'Safety B: Ambiguous first name ("Craig") requires clarification',
      () async {
        final query = const SearchQuery(
          intent: SearchIntent.searchDriverVideos,
          driverName: 'Craig',
        ...
        expect(res.candidates.length >= 2, isTrue);
        expect(
          res.candidates.any((c) => c.canonicalName == 'Craig Breen'),
          isTrue,
        );
        expect(
          res.candidates.any((c) => c.canonicalName == 'Craig MacWilliam'),
          isTrue,
        );
      },
    );
    ...
    test('Safety D: Multi-edition rally WITH year ("Moonraker" + 2025) resolves uniquely to 2025 edition', () async {
      ...
      expect(
        res.resolvedRallyCandidate?.canonicalName,
        equals('Moonraker Forestry Rally 2025'),
      );
    ...
    test('Safety E: Completely unrelated entity produces no match', () async {
      final query = const SearchQuery(
        intent: SearchIntent.searchDriverVideos,
        driverName: 'Banana Pineapple Spaceship',
      );
      final res = await resolver.resolve(query);
